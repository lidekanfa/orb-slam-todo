     ...
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
    ...
    // Set no stereo information  这种赋值方法
    ...
    if(mbInitialComputations)
    {
        ...
        mfGridElementWidthInv=static_cast<float>(FRAME_GRID_COLS)/static_cast<float>(mnMaxX-mnMinX);
        mfGridElementHeightInv=static_cast<float>(FRAME_GRID_ROWS)/static_cast<float>(mnMaxY-mnMinY);///后面可以用来判断在哪个格子里面，和角度直方图用法一样
...
{
    //每个格子中提取特征点的个数
    int nReserve = 0.5f*N/(FRAME_GRID_COLS*FRAME_GRID_ROWS);///一半的特征点是几个意思，可能也是用来加速的，防止不断改变容器大小的时候浪费时间，其次一半也是为了节约空间，毕竟大部分格子里面的特征点没有那么多
    ...
    for(int i=0;i<N;i++)
    {
        ...
        int nGridPosX, nGridPosY;//格子索引
        if(PosInGrid(kp,nGridPosX,nGridPosY))//如果这一点在有效范围内，则在相应的格子里记录下来
...
bool Frame::isInFrustum(MapPoint *pMP, float viewingCosLimit)
{
    ...
    const float maxDistance = pMP->GetMaxDistanceInvariance();///如何判断的？
    const float minDistance = pMP->GetMinDistanceInvariance();
    // 世界坐标系下，相机到3D点P的向量, 向量方向由相机指向3D点P
    const cv::Mat PO = P-mOw;///这就是计算的相对距离，不用转换到世界坐标系下吧
    ...
    cv::Mat Pn = pMP->GetNormal();///这就取得平均视角夹角了？

    const float viewCos = PO.dot(Pn)/dist;//归一化
    ...
    const int nPredictedLevel = pMP->PredictScale(dist,this);///这操作有点秀，咋整的？
...
vector<size_t> Frame::GetFeaturesInArea(const float &x, const float  &y, const float  &r, const int minLevel, const int maxLevel) const
{
    ...
    const int nMinCellX = max(0,(int)floor((x-mnMinX-r)*mfGridElementWidthInv));//格子搜索范围的左边界
    if(nMinCellX>=FRAME_GRID_COLS)
        return vIndices;

    const int nMaxCellX = min((int)FRAME_GRID_COLS-1,(int)ceil((x-mnMinX+r)*mfGridElementWidthInv));//格子搜索范围的又边界
    if(nMaxCellX<0)
        return vIndices;

    const int nMinCellY = max(0,(int)floor((y-mnMinY-r)*mfGridElementHeightInv));//格子搜索范围的上边界
    if(nMinCellY>=FRAME_GRID_ROWS)
        return vIndices;

    const int nMaxCellY = min((int)FRAME_GRID_ROWS-1,(int)ceil((y-mnMinY+r)*mfGridElementHeightInv));//格子搜索范围的下边界
    if(nMaxCellY<0)
        return vIndices;
    ...
    for(int ix = nMinCellX; ix<=nMaxCellX; ix++)
    {
        for(int iy = nMinCellY; iy<=nMaxCellY; iy++)
        {
            ...
            for(size_t j=0, jend=vCell.size(); j<jend; j++)//划分格子后也方便操作
            {
                const cv::KeyPoint &kpUn = mvKeysUn[vCell[j]];///这是什么鬼，不是应该在这个格子里面找特征点吗？
                if(bCheckLevels)
                {
...
void Frame::UndistortKeyPoints()
{
    ...
    for(int i=0; i<N; i++)
    {
        cv::KeyPoint kp = mvKeys[i];///你这多一步赋值干啥．没有用

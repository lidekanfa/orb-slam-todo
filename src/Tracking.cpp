...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    {
        if(mbRGB)
            //cvtColor函数将一个色彩空间转换到另一个色彩空间
            //参数1输入图像 参数2输出图像 参数3色彩空间int类型
            cvtColor(mImGray,mImGray,CV_RGB2GRAY);
        else
            cvtColor(mImGray,mImGray,CV_BGR2GRAY);
    }
    ...
    else// 步骤2：跟踪
    {
        ...
        else
        {
            ...
            else
            {
                ...
                // mbVO为false表示上帧匹配了很多的MapPoints，跟踪很正常，
                // mbVO为true表明上帧匹配了很少的MapPoints，少于10个，要跪的节奏
                if(!mbVO)
                {
                ...
                else
                {
                    ...
                    if(bOKMM && !bOKReloc)
                    {
                        ...
                        if(mbVO)
                        {
                            // 这段代码是不是有点多余？应该放到TrackLocalMap函数中统一做 它进不去啊同学
...
void Tracking::MonocularInitialization()
{
    ...
    if(!mpInitializer)
    {
        ...
        if(mCurrentFrame.mvKeys.size()>100)
        {
            ...
            // 记录最近的一帧
            mLastFrame = Frame(mCurrentFrame);
            // mvbPrevMatched最大的情况就是所有特征点都被跟踪上 mvKeysUn是特征点
            mvbPrevMatched.resize(mCurrentFrame.mvKeysUn.size());
            //第一帧假设所有的关键点都被匹配上，记录匹配点的坐标（图像上）？
    ...
    else
    {
        ...
        ORBmatcher matcher(0.9,true);  //0.9为最近邻和次近邻距离之比  true为是否检查相机朝向
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    // 步骤4:通过优化3D-2D的重投影误差来获得位姿  将地图点根据上一帧的pose投影到当前帧，计算投影误差，不断优化
...
bool Tracking::Relocalization()
{
    ...
    //这样写有没有可能出现候选帧到最后没有被置0，
    // 并且剩下的候选帧中内点个数少于要求的内点个数，
    //以至于循环一直在进行
